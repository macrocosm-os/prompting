...
def create_task(llm_pipeline, name, **kwargs):
    if name == "summarization":
        return SummarizationTask(
            llm_pipeline=llm_pipeline, context=wiki_dataset.next(), **kwargs
        )
        ...
        return DebuggingTask(
            llm_pipeline=llm_pipeline, context=code_dataset.next(), **kwargs
        )

    elif name == "date":
        return DateQuestionAnsweringTask(
            llm_pipeline=llm_pipeline, context=date_dataset.next(), **kwargs
        )

    elif name == "math":
        return MathTask(
            llm_pipeline=llm_pipeline, context=math_dataset.next(), **kwargs
        )
...
for i in tqdm(range(n_trials)):
    ...
    for sys_prompt_template in sys_prompt_templates:
        ...
        agent = HumanAgent(
            ...
            system_template=sys_prompt_template,
        )
        ...
        if eval:
            ...
            message = gpt_judge_system_prompt.format(
                system=agent.system_prompt, user=agent.challenge
            )
        ...
        for j in tqdm(range(n_references)):
            ...
            t0 = time.time()
            model_reference = HuggingFaceLLM(
                llm_pipeline, system_prompt=miner_completion_system_prompt
            ).query(agent.challenge)
            model_reference_time = time.time() - t0

            eval_dict = {}
            if eval:
                # bt.logging.info("🤖 Scoring reference using GPT...")
                message = gpt_reference_eval_prompt.format(
                    challenge=agent.challenge, reference=task.reference
                )
                reference_from_challenge_eval = gpt_judge(message)

                # bt.logging.info("🤖 Scoring reference using GPT...")
                message = gpt_reference_eval_prompt.format(
                    challenge=task.query, reference=task.reference
                )
                ...
                eval_dict = {
                    ...
                    "reference_from_challenge_eval_raw": reference_from_challenge_eval[
                        "response"
                    ],
                    "reference_from_challenge_eval_score": reference_from_challenge_eval[
                        "score"
                    ],
                    "reference_from_query_eval_raw": reference_from_query_eval[
                        "response"
                    ],
                    "reference_from_query_eval_score": reference_from_query_eval[
                        "score"
                    ],
                }
