...
class OpenAIMiner:
    ...
    async def create_chat_completion(self, request: Request):
        ...
        if request.headers.get("task", None) == "WebRetrievalTask":
            ...
            return await self.create_inference_completion(data.get("messages"))

    async def create_inference_completion(self, messages):
        async def word_stream():
            inference = await self.inference(messages)
    ...
    async def check_availability(self, request: Request):
        ...
        model_response = {key: True for key in llm_model_availabilities}
