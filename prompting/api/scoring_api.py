...
@app.post("/scoring")
async def scoring_endpoint(request: Request, verified: bool = Depends(verify_api_key)):
    try:
        ...
        if body.get("task") != "InferenceTask":
            logger.warning(
                "A non inference task has been received, which is not supported for organic scoring. Skipping..."
            )
        else:
            try:
...
async def start_scoring_endpoint():
    if not settings.SCORING_ENDPOINT_HOST and settings.SCORING_ENDPOINT_PORT:
        logger.exception(
            "If you are deploying an api you must set SCORING_ENDPOINT_HOST and SCORING_ENDPOINT_PORT in the .env.validator"
        )
        raise RuntimeError
    uvicorn.run(
        app,
