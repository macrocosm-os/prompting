...
def contains_gpu_index_in_device(device: str) -> bool:
    pattern: str = r"^cuda:\d+$"
    return bool(re.match(pattern, device))


def calculate_single_gpu_requirements(
    device: str, max_allowed_memory_allocation_in_bytes: int
) -> float:
    ...
    gpu_utilization: float = round(
        max_allowed_memory_allocation_in_bytes / global_free, 2
    )
    bt.logging.info(
...
def calculate_multiple_gpu_requirements(
    device: str, gpus: int, max_allowed_memory_allocation_in_bytes: int
) -> float:
    torch.cuda.synchronize()
    total_free_memory = 0
    total_gpu_memory = 0
    ...
    bt.logging.info(
        f"Total GPU memory across all visible GPUs: {gpus} {round(total_gpu_memory / 10e8, 2)} GB"
    )
    ...
    gpu_utilization: float = round(
        max_allowed_memory_allocation_in_bytes / total_free_memory, 2
    )
    bt.logging.info(
...
def calculate_gpu_requirements(
    ...
    """
    if gpus == 1:
        return calculate_single_gpu_requirements(
