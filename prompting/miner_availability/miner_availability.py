...
class MinerAvailability(BaseModel):
    """Keeps track of one miner's availability."""
...
class MinerAvailabilities(BaseModel):
    """Track of all the miner's availabilities and allows targeting miners based on their availability."""

    miners: dict[int, MinerAvailability] = {}

    def set_all_miners_available(self, uids: np.ndarray):
        """Set all given miners to be available for all tasks and models."""
        for uid in uids:
            self.miners[uid] = MinerAvailability(
...
class CheckMinerAvailability(AsyncLoopRunner):
    ...
    async def run_step(self):
        if shared_settings.DEBUG:
            # If in DEBUG mode, skip querying and make everyone available.
            logger.warning("DEBUG mode is ON. Skipping miner availability checks and marking all as available.")
            miner_availabilities.set_all_miners_available(self.uids)
            await asyncio.sleep(1.0)
            return

        # Normal behavior if not in debug mode
