...
class RelevanceRewardModel(BaseRewardModel):
    ...
    async def reward(
        self, reference: str, response_event: DendriteResponseEvent, angle_model: AnglE, **kwargs
    ) -> BatchRewardOutput:
        ...
        """
        reference_embedding = angle_model.encode(reference, to_numpy=True)
        ...
        baseline = 1 - float(
            spatial.distance.cosine(reference_emb_flatten, angle_model.encode("", to_numpy=True).flatten())
        )
        ...
        for comp in completions:
            if len(comp) == 0:
                ...
                continue
            t0 = time.time()
            emb = angle_model.encode(comp, to_numpy=True)
