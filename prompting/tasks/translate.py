...
class TranslationPipeline:
    ...
    def load_translation_packages(self) -> list[AvailablePackage]:
        ...
        def is_package_installed(from_code, to_code, packages) -> bool:
            return any(
                pkg
                for pkg in packages
        ...
        supported_language_pairs = [
            ...
            if pkg.from_code in SUPPORTED_LANGUAGES
            and pkg.to_code in SUPPORTED_LANGUAGES
        ]
        ...
        for package in pbar:
            if not is_package_installed(
                package.from_code, package.to_code, installed_packages
            ):
                bt.logging.info(
                    f"Installing package from {package.from_code} to {package.to_code}"
                )
    ...
    def translate_to_random_language(
        self, content: str, from_code: str = "en"
    ) -> Tuple[AvailablePackage, str]:
        english_supported_languages = list(
            filter(lambda x: x.from_code == from_code, self.supported_language_pairs)
        )
        ...
        translated_content = argostranslate.translate.translate(
            content, from_code, to_code=translation_code
        )
...
class TranslationTask(Task):
    ...
    def __init__(self, translation_pipeline: TranslationPipeline, context: Context):
        ...
        content_translation_obj, translated_content = (
            translation_pipeline.translate_to_random_language(context.content)
        )

        # Translates the translation to another random language
        reference_translation_obj, reference_translation_content = (
            translation_pipeline.translate_to_random_language(
        ...
        self.query = template.format(
            another_language=reference_translation_obj.to_name, text=translated_content
        )
