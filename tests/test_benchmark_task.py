...
def test_extract_query_and_reference_with_successful_generations(
    ...
    task = MultiChoiceTask(llm_pipeline=mock_llm_pipeline(message), context=MOCK_CONTEXT)
...
def test_extract_query_and_reference_with_missing_correct_answer_marker(message: str, reference: str):
