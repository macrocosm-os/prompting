...
sys.argv = [__file__, "--mock", "--neuron.tasks", "qa"]
...
def test_generate_reference_parallel_to_dendrite(generate_reference_time, dendrite_time, expected_forward_time):
    ...
    reward_pipeline_time = sum(event[key] for key in event if key.endswith("batch_time"))
    network_and_reference_gen_time = step_time - reward_pipeline_time

    # TODO: Fix unit test to work with abs=0.1
    assert network_and_reference_gen_time == pytest.approx(expected_forward_time, abs=1)  # 0.1
